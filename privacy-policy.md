...
- **Page content (limited)** — When you use **organize by category**, the Extension injects a **bundled script** into web pages you already have open to read public metadata (for example Open Graph and JSON-LD) and hostname signals. This is used only to suggest category groupings and previews. The Extension does **not** read passwords, form fields, or cookies.
...
- **Host access (http/https)** — Required so the injected script can run on the same pages you are organizing. The Extension does **not** fetch and execute remote code from the internet.
